...
dirt <- dirt %>%
  mutate(
    ...
    nearest_warehouse = recode_factor(nearest_warehouse,
                                      "nickolson" = "Nickolson",
                                      "thompson" = "Thompson"
    ),
    shopping_cart = recode_factor(shopping_cart,
                                  "[('IASSIST LINE', 2), ('lucent 330s', 2)]" = "[('iAssist Line', 2), ('Lucent 330S', 2)]",
    ...
    season = case_when(
      month(date) %in% 1:3 ~ "Winter",
      month(date) %in% 4:6 ~ "Spring",
      month(date) %in% 7:9 ~ "Summer",
      month(date) %in% 10:12 ~ "Autumn",
      TRUE ~ NA_character_
    )
  )
